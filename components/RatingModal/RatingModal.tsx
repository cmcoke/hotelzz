    ...
    <div className={`fixed z-[61] inset-0 flex items-center justify-center ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      <div className="bg-white w-96 p-4 rounded-lg shadow-lg">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center">
            {starValues.map(value => (
              <button className={`w-6 h-6 ${ratingValue === value ? "text-yellow-500" : "text-gray-300"}`} onClick={() => setRatingValue(value)} key={value}>
                <BsStarFill />
              </button>
            ))}
          </div>
        ...
        <div className="flex justify-end">
          <button onClick={reviewSubmitHandler} className="px-4 py-2 bg-primary text-white rounded-md" disabled={isSubmittingReview}>
